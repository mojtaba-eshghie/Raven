...
# Random state for thesis:
RANDOM_STATE = 15

# Random state for ReBERT:
# RANDOM_STATE = 36
...
def get_rand_trans_multi(transaction_nr, input_file, output_file):
    general_logger = setup_logger()
    general_logger.info(f"Start running for {transaction_nr} transactions with random state: {RANDOM_STATE}")
    
    start_time = time.time()
    df = pd.read_parquet(input_file)
    random_hashes = df.sample(n=transaction_nr, random_state=RANDOM_STATE)["hash"]
...
def main():
    ...
    parser.add_argument("--input", type=str, default=r"ethereum_failed_transactions\dune_results\all_hashes.parquet",
                        help="Path to input parquet file")
    parser.add_argument("--output", type=str, default="datasets/test.parquet",
