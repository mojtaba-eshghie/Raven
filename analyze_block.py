...
def get_rand_trans_multi(transaction_nr, input_file, log_file, output_file):
    ...
    start_time = time.time()
    df = pd.read_parquet(input_file)
    random_rows = df.sample(n=transaction_nr, random_state=15)  # Set random_state for reproducibility    
    cols = random_rows["hash"]
    batch = []
    BATCH_SIZE = 1000
...
def run_all_mutli(input_file, log_file, output_file):
    general_logger = logging.getLogger("main_logger")
    if not general_logger.hasHandlers():
        handler = logging.FileHandler(log_file)
        handler.setFormatter(logging.Formatter('%(asctime)s - %(levelname)s - %(message)s'))
